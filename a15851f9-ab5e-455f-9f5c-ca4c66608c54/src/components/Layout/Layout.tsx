...
const Layout: React.FC<LayoutProps> = ({ appConfig }) => {
  ...
  init({
    ...
    getAuthToken: () => {
      // return fetch('http://10.79.137.196:3001/api/apps/accessToken', {
      //   method: 'GET',
      //   headers: {
  ...
  document.documentElement.style.setProperty(
    '--var-root-background',
    appConfig.color_schema['--ts-var-root-background'] || '#F6F8FA'
  );
  document.documentElement.style.setProperty(
    '--var-viz-background',
    appConfig.color_schema['--ts-var-viz-background'] || '#FFFFFF'
  );
  document.documentElement.style.setProperty(
    '--var-root-color',
    appConfig.color_schema['--ts-var-root-color'] || '#000000'
  );
  document.documentElement.style.setProperty(
    '--var-button--secondary-background',
    appConfig.color_schema['--ts-var-button--secondary-background'] || '#2770EF'
  );
