...
const Layout: React.FC<LayoutProps> = ({ config }) => {
  ...
  init({
    ...
    customizations: {
      style: {
        customCSS: {
          variables: config?.app_config?.color_schema || {},
        },
      },
    },
    getAuthToken: () => {
    ...
    document.documentElement.style.setProperty(
      '--var-viz-background',
      config?.app_config?.color_schema?.['--ts-var-viz-background'] || '#FFFFFF'
    );
    document.documentElement.style.setProperty(
      '--var-root-color',
      config?.app_config?.color_schema?.['--ts-var-root-color'] || '#000000'
    );
    document.documentElement.style.setProperty(
      '--var-button--secondary-background',
      config?.app_config?.color_schema?.['--ts-var-button--secondary-background'] || '#2770EF'
    );

  const getTabContent = () => {
    if (activeTab === 'dashboard') {
      return <LiveBoard liveboardId={config.liveboardId} />;
    }
    return <Spotter modelId={config.modelId} />;
  };
  const extractItems = config?.app_config?.menu_items?.map((menuItem: never, index: number) => {
    return {
      href: '#' + menuItem,
      label: menuItem,
      isActive: index == 1,
  ...
  return (
    <div className={styles.layout}>
      <Header logo={config.logo} menuItems={extractItems} />
