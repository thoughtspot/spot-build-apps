...
const Liveboard: React.FC<LiveboardProps> = ({ liveboardId, colorSchema }) => {
  ...
  useEffect(() => {
    console.log('[Liveboard] Component mounted:', {
      liveboardId,
      hasLiveboardId: !!liveboardId,
      liveboardIdLength: liveboardId?.length,
      liveboardIdType: typeof liveboardId,
      hasRef: !!ref.current,
      colorSchemaKeys: colorSchema ? Object.keys(colorSchema) : [],
    });
  }, [liveboardId, colorSchema, ref]);

  useEffect(() => {
    if (ref.current) {
      console.log('[Liveboard] Ref updated, embed instance available:', {
        hasRef: !!ref.current,
