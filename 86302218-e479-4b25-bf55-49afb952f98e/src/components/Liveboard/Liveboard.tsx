...
const Liveboard: React.FC<LiveboardProps> = ({ liveboardId, colorSchema }) => {
  ...
  React.useEffect(() => {
    console.log('[Liveboard] Component rendered:', {
      liveboardId,
  ...
  if (!liveboardId) {
    console.warn('[Liveboard] No liveboardId provided');
    return (
      <div style={{ padding: '20px', textAlign: 'center' }}>
        <p>Liveboard ID is missing</p>
        <pre>{JSON.stringify({ liveboardId }, null, 2)}</pre>
      </div>
    );
  }
