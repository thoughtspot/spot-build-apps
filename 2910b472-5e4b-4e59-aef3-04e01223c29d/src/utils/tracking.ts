...
// In published apps, frontend + backend are typically served from the same origin, so default to same-origin `/api`.
// In local dev, keep the old localhost fallback unless VITE_API_BASE_URL is provided.
const API_BASE_URL =
...
export const getCommonProperties = (appId?: string): Record<string, unknown> => {
  return {
    ...
    // Server-side Mixpanel query expects `app_id`
    app_id: appId || '',
    // Keep camelCase as well for any downstream usage
    appId: appId || '',
  };
};
