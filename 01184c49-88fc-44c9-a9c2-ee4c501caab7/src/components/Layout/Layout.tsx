...
const Layout: React.FC<LayoutProps> = ({ config }) => {
  ...
  const [, setIsSDKInitialized] = useState(false);
  ...
  useEffect(() => {
    if (config?.host) {
      try {
        ...
        if (authEventEmitter) {
          authEventEmitter.on(AuthStatus.SUCCESS, () => setIsSDKInitialized(true));
          authEventEmitter.on(AuthStatus.SDK_SUCCESS, () => setIsSDKInitialized(true));
        }
      } catch (error) {
